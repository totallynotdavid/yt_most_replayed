async function waitForAdSkip(videoPage, maxWaitTime = 15000) {
  ...
  while (true) {
    ...
    if (Date.now() - startTime > maxWaitTime) {
      console.log('Maximum wait time exceeded. Proceeding with non-skippable ad or no ad.');
      break;
    }

    try {
      const skipButton = await videoPage.$('.ytp-ad-skip-button', { timeout: 1000 }).catch(() => null);
      if (skipButton) {
